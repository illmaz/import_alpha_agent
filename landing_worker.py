"""Landing agent: consumes its assigned tasks and emits artifact.created."""
...
def handle(event: Event, topic: str) -> None:
    ...
    print(f"[landing] Landing Agent is working on {event.task_id}: {title}")
    time.sleep(WORK_SECONDS)

    artifact = Event(
        event_type="artifact.created",
        task_id=event.task_id,
        agent="landing_worker",
        payload={
            "artifact_type": "landing_brief",
            "summary": f"Placeholder landing brief for {event.task_id} ({title}).",
            "status": "stub",
    ...
    print(f"[landing] artifact.created  {event.task_id}  ->  '{OUTPUT_TOPIC}'")
