"""Product agent: consumes its assigned tasks and emits artifact.created."""
...
def handle(event: Event, topic: str) -> None:
    ...
    print(f"[product] Product Agent is working on {event.task_id}: {title}")
    ...
    artifact = Event(
        ...
        payload={
            ...
            "summary": f"Placeholder product brief for {event.task_id} ({title}).",
            "status": "stub",
    ...
    print(f"[product] artifact.created  {event.task_id}  ->  '{OUTPUT_TOPIC}'")


def main() -> None:
    logging.basicConfig(level=logging.INFO, format="%(levelname)s %(name)s: %(message)s")
    install_signal_handlers("product")
    print(f"[product] listening on '{SOURCE_TOPIC}' (group={GROUP_ID})")
